...
Creates all necessary tables, indexes, and RLS policies for the PetBnB application
"""
...
async def create_database_schema():
    ...
    try:
        ...
        statements = table_creation_sql.split(';')
        for statement in statements:
            if statement.strip():
                result = await client.rpc('exec_sql', {'sql': statement.strip()}).execute()
                logger.info(f"✓ Table creation statement executed")
        
        logger.info("Creating database indexes...")
        
        # Split and execute index statements
        index_statements = indexes_sql.split(';')
        for statement in index_statements:
            if statement.strip():
                result = await client.rpc('exec_sql', {'sql': statement.strip()}).execute()
                logger.info(f"✓ Index creation statement executed")
